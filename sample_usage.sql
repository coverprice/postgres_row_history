...
*/
BEGIN;
SELECT changelog_new('Change-001 insert some new test data', 'user_1234');
...
BEGIN;
SELECT changelog_new('Change-002 deleting a row', 'user_1234');
...
BEGIN;
SELECT changelog_new('Change-003 updating a row', 'user_1234');
...
BEGIN;
SELECT changelog_new('Change-004 updating a row, seeing if columns are ignored', 'user_1234');
...
BEGIN ;
SELECT changelog_new('Change-005 truncating the whole table', 'user_1234');
TRUNCATE test_data;
COMMIT;
