...
BEGIN;
SELECT changeset_new('Change-001 insert some new test data. Expect 3 change_history log rows.', 'user_1234');
...
BEGIN ;
SELECT changeset_new('Change-002 UPDATE has no effect (data is updated to its current value). Expect no change_history logs.', 'user_1234');
...
-- NB: this will still create a changeset entry, but no actual changes will be logged against it.
BEGIN ;
SELECT changeset_new('Change-003 UPDATE _only_ affects a column that is configured to be ignored. Expect no change_history logs.', 'user_1234');
...
BEGIN;
SELECT changeset_new('Change-004 DELETE a row. Expect 1 change_history log row.', 'user_1234');
...
BEGIN;
SELECT changeset_new('Change-005 UPDATE a column in a single row. Expect 1 change_history log row.', 'user_1234');
...
BEGIN;
SELECT changeset_new('Change-006 UPDATE multiple columns (int & float). Expect 1 change_history log row.', 'user_1234');
...
-- of the comparison of the before/after column values)
BEGIN;
SELECT changeset_new('Change-007 UPDATE a JSONB column. Expect 1 change_history log row.', 'user_1234');
...
BEGIN;
SELECT changeset_new('Change-008 UPDATE multiple columns in a single row. Some columns are configured to be ignored. Expect 1 change_history log row.', 'user_1234');
...
BEGIN ;
SELECT changeset_new('Change-009 truncating the whole table. Expect 2 change_history log rows.', 'user_1234');
